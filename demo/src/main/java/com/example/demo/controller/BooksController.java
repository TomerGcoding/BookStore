...
//
//    @GetMapping("/book")
//    public ResponseEntity<Map<String, Object>> getBookById(@RequestParam(name = "id") Long id, @RequestParam(name = "persistenceMethod") String persistence) {
//        long requestId = requestCounter++;
//        long startTime = System.currentTimeMillis();
//        requestLogger.info("Incoming request | #{} | resource: /book | HTTP Verb GET | request #{}", requestId,requestId);
//
//        try {
//            Book book = bookService.getBookById(id,persistence).orElseThrow(() -> new Exception("Error: no such Book with id " + id));
